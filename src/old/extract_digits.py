    ...
    for y in range(9):
        for x in range(9):
            ...
            (text_width, text_height) = \
                cv2.getTextSize(digit, font_base, fontScale=font_scale_small, thickness=thickness_small)[0]
            cv2.putText(im_filled_grid, digit,
                        (true_x - int(text_width / 2), true_y + int(text_height / 2)),
                        font_base, font_scale_small, (0, 3, 0), thickness_small * 3)
            cv2.putText(im_filled_grid, digit,
                        (true_x - int(text_width / 2), true_y + int(text_height / 2)),
                        font_base, font_scale_small, (0, 0, 255), thickness_small)
...
if __name__ == '__main__':
    model_ = load_model('model/my_model.h5')
    ...
    cv2.imshow("im", im)
    res_grids = process_extract_digits([im], model_,
